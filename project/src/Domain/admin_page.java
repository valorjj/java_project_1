...
public class admin_page {
		...
		if (admin_logIn()) { // 1. 아이디 & 비밀번호가 일치해야 관리자 메뉴 실행시킵니다.
			...
			while (run) {
				...
				// 2. 관리자가 메뉴를 수정합니다.
					...
					int adminSelect4 = main.scanner.nextInt();
					if (adminSelect4 == 1) {
				...
				// 3. 관리자가 메뉴를 삭제합니다.
					...
					int adminSelect3 = main.scanner.nextInt();
					if (adminSelect3 == 1) {
	...
	public static boolean adminCoffeeCheck(String name) {
		for (int i = 0; i < main.coffeeList.length; i++) {
			...
			if (main.coffeeList[i] != null && main.coffeeList[i].getCoffeeType().equals(name)) {
				// 둘중 하나라도 겹치는 경우에는
	...
	public static boolean adminDessertCheck(String name) {
		for (int i = 0; i < main.dessertList.length; i++) {
			if (main.dessertList[i] != null && main.dessertList[i].getDessert_name().equals(name)) {
				// 둘중 하나라도 겹치는 경우에는
				System.out.println("중복입니다. ");
				return false; // 중복인 경우에는 false 를 리턴합니다.
			}
	...
	public static void adminDeleteCoffeeMenu() {
		...
		String c_name = main.scanner.next();
		for (int i = 0; i < main.coffeeList.length; i++) {
			if (main.coffeeList[i] != null && main.coffeeList[i].equals(c_name)) { // 해당 커피 메뉴를 찾습니다.
				main.coffeeList[i] = null;
			}
		}
	...
	public static void adminDeleteDessertMenu() {
		...
		String d_name = main.scanner.next();
		for (int i = 0; i < main.dessertList.length; i++) {
			if (main.dessertList[i] != null && main.dessertList[i].equals(d_name)) { // 해당 커피 메뉴를 찾습니다.
				main.dessertList[i] = null;
			}
		}
	}
	...
	public static void adminCoffeeList() {
		for (int i = 0; i < main.coffeeList.length; i++) {
			if (main.coffeeList[i] != null) {
				System.out.println("커피 이름 : " + main.coffeeList[i].getCoffeeType());
				System.out.println("커피 가격 : " + main.coffeeList[i].getCoffeePrice());
				System.out.println("커피 재고: " + main.coffeeList[i].getCoffeeStock());
			}
		}
	}
	...
	public static void adminDessertList() {
		for (int i = 0; i < main.dessertList.length; i++) {
			if (main.dessertList[i] != null) {
				System.out.println("디저트 이름 : " + main.dessertList[i].getDessert_name());
				System.out.println("디저트 가격 : " + main.dessertList[i].getDessert_price());
				System.out.println("디저트 재고 : " + main.dessertList[i].getDessert_stock());
			}
		}
	}
	...
	public static void adminCoffeeModify() {
		...
		String c_name = main.scanner.next();
		for (int i = 0; i < main.coffeeList.length; i++) {
			if (main.coffeeList[i] != null && main.coffeeList[i].getCoffeeType().equals(c_name)) {
				System.out.print("수정할 가격 : ");
				String c_price = main.scanner.next();
				System.out.print("수정할 재고 : ");
				String c_stock = main.scanner.next();
				coffee = new coffee_menu(c_name, c_price, c_stock);
				main.coffeeList[i] = coffee; // 새로 입력받은 값으로 덮어씌웁니다.
			}
	...
	public static void adminDessertModify() {
		...
		String d_name = main.scanner.next();
		for (int i = 0; i < main.dessertList.length; i++) {
			if (main.dessertList[i] != null && main.dessertList[i].getDessert_name().equals(d_name)) {
				System.out.print("수정할 가격 : ");
				String d_price = main.scanner.next();
				System.out.print("수정할 재고 : ");
				String d_stock = main.scanner.next();
				dessert = new dessert_menu(d_name, d_price, d_stock);
				main.dessertList[i] = dessert; // 새로 입력받은 값으로 덮어씌웁니다.
			}
		}
	}
